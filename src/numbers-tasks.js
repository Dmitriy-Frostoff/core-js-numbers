...
function getCircleCircumference(radius) {
  ...
  if (radius < 0) {
    throw new Error(`radius must be positive number! Given ${radius}`);
  }
...
function getAverage(value1, value2) {
  if (!Number.isFinite(value1) || !Number.isFinite(value2)) {
    throw new Error(`Given not a number values`);
  }
...
function getLastDigit(value) {
  ...
  return value % 10 ** (digitCount > 1 ? digitCount - 1 : digitCount);
}
...
function getSumToN(n) {
  ...
  if (!Number.isFinite(n)) {
    throw new Error('Not a Finite value was given!');
  }
...
function isPowerOfTwo(num) {
  if (!Number.isFinite(num)) {
    throw new Error('Number must be finite integer!');
  }
...
function getSumOfNumbers(x1, x2, x3) {
  if (!Number.isFinite(x1) || !Number.isFinite(x2) || !Number.isFinite(x3)) {
    throw new Error('Not a number was given!');
  }
...
function getRandomInteger(min, max) {
  if (!Number.isInteger(min) || !Number.isInteger(max)) {
    throw new Error('Not an ineger numbers were given');
  }
